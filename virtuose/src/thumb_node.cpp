...
{
  printf("Starting THUMB node\n");
  // Init ROS library
  ros::init(argc, argv, "THUMB");
  // Create node handle
  ros::NodeHandle n;

  // Create topic publishers
  printf("Creating topics\n");
  ros::Publisher out_virtuose_status = n.advertise<virtuose::out_virtuose_status>("out_thumb_status", 1);
  _out_virtuose_status = &out_virtuose_status;
  ros::Publisher out_virtuose_pose = n.advertise<virtuose::out_virtuose_pose>("out_thumb_pose", 1);
  _out_virtuose_pose = &out_virtuose_pose;

  ros::Publisher out_virtuose_articular_pose = n.advertise<sensor_msgs::JointState>("out_thumb_articular_pose", 1);
  _out_virtuose_articular_pose = &out_virtuose_articular_pose;

  ros::Publisher out_virtuose_physical_pose = n.advertise<virtuose::out_virtuose_physical_pose>("out_thumb_physical_pose", 1);
  _out_virtuose_physical_pose = &out_virtuose_physical_pose;
  ros::Publisher out_virtuose_speed = n.advertise<virtuose::out_virtuose_speed>("out_thumb_speed", 1);
  _out_virtuose_speed = &out_virtuose_speed;
  ros::Publisher out_virtuose_force = n.advertise<virtuose::out_virtuose_force>("out_thumb_force", 1);
  _out_virtuose_force = &out_virtuose_force;
  // Create topic subscribers
  ros::Subscriber in_virtuose_pose = n.subscribe("in_thumb_pose", 1, in_virtuose_poseCB);

  ros::Subscriber in_virtuose_speed = n.subscribe("in_thumb_speed", 1, in_virtuose_speedCB);
  ros::Subscriber in_virtuose_force = n.subscribe("in_thumb_force", 1, in_virtuose_forceCB);

  // Create services
  printf("Creating services\n");

  ros::ServiceServer impedance = n.advertiseService("thumb_impedance", impedanceCB);
