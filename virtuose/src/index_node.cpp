...
int main(int argc, char **argv)
{
  ...
  ros::Publisher out_virtuose_status = n.advertise<virtuose::out_virtuose_status>("out_index_status", 1);
  _out_virtuose_status = &out_virtuose_status;
  ros::Publisher out_virtuose_pose = n.advertise<virtuose::out_virtuose_pose>("out_index_pose", 1);
  _out_virtuose_pose = &out_virtuose_pose;
  ros::Publisher out_virtuose_physical_pose = n.advertise<virtuose::out_virtuose_physical_pose>("out_index_physical_pose", 1);
  ros::Publisher out_virtuose_articular_pose = n.advertise<sensor_msgs::JointState>("out_index_articular_pose", 1);
  _out_virtuose_articular_pose = &out_virtuose_articular_pose;
  _out_virtuose_physical_pose = &out_virtuose_physical_pose;
  ros::Publisher out_virtuose_speed = n.advertise<virtuose::out_virtuose_speed>("out_index_speed", 1);
  _out_virtuose_speed = &out_virtuose_speed;
  ros::Publisher out_virtuose_force = n.advertise<virtuose::out_virtuose_force>("out_index_force", 1);
  _out_virtuose_force = &out_virtuose_force;
  // Create topic subscribers
  ros::Subscriber in_virtuose_pose = n.subscribe("in_index_pose", 1, in_virtuose_poseCB);
  ros::Subscriber in_virtuose_speed = n.subscribe("in_index_speed", 1, in_virtuose_speedCB);
  ros::Subscriber in_virtuose_force = n.subscribe("in_index_force", 1, in_virtuose_forceCB);

  // Create services
  printf("Creating services\n");

  ros::ServiceServer impedance = n.advertiseService("index_impedance", impedanceCB);
